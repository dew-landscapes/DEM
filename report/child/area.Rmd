...
terr <- l[results_df$aoi_name == areaID] |>
  purrr::map(\(x) {
  ...
  terra::rast() |>
  setNames(paste0(results_df$dem_name[results_df$aoi_name == areaID]))
...
```{r categ{{areaID}}, fig.cap = paste0("Categorical representation of ", areaID)}

  terra::panel(terr)

```

```{r categCount{{areaID}}, fig.cap = paste0("Proportion of categorised ", areaID, " area in each of several classification classes")}
